...
def _get_contact_db_path():
    """Get contact.db path, checking source freshness on every call.

    Prefers live-decrypted (mtime-checked by DBCache) over the static
    pre-decrypted copy — the latter can lag behind newly-added contacts.
    Invalidates all in-memory contact caches when re-decryption occurred.
    """
